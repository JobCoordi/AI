...
class FinalRecommendation:
    def __init__(self, model_name: str = "gpt-4o", db_connection: Optional[str] = None):
        ...
        if not self.db_connection:
            ...
            raise ValueError("데이터베이스 연결 문자열이 제공되지 않았습니다.")
...
"""
    ...
    def _extract_user_messages(self, messages: List[Any]) -> List[str]:
        ...
        # ✅ 중복 제거
        return list(dict.fromkeys(user_inputs))
    ...
    def get_recommendation(self, user_id: str) -> str:
        try:
            ...
            if not past_messages:
                ...
                return "충분한 대화 기록이 없어 추천을 제공할 수 없습니다."
            ...
            prompt = self.final_prompt_template.format_prompt(messages=messages_combined)
            ...
            if isinstance(response, AIMessage):
                ...
                recommendation = str(response).strip()

            # ✅ 포맷 검증
            if recommendation.count(":") != 2:
                self.logger.warning(f"추천 결과 형식 오류: {recommendation}")
                return "추천 결과 형식이 올바르지 않습니다. 다시 시도해 주세요."
            ...
            return "데이터베이스 연결 문제로 추천을 제공할 수 없습니다."
